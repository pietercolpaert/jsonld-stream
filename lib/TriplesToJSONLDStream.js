...
function TriplesToJSONLDStream (context) {
  ...
  process.nextTick(function () {
    self.push({"@context": this["@context"]});
  });
...
TriplesToJSONLDStream.prototype._flush = function (done) {
  this._createObject(this._jsonld, done);
};
...
TriplesToJSONLDStream.prototype._coupleObjects = function (jsonldGraph) {
  ...
  for (var i = 0; i < jsonldGraph["@graph"].length; i++) {
    ...
    if (typeof temp[jsonldGraph["@graph"][i]["@id"]] === "undefined") {
      ...
      //If the element does exist, merge the elements: it will contain an @type, or a specific predicate, as we have 1 array element per 1 triple
      for (var key in jsonldGraph["@graph"][i]) {
        //If the key already exists, and if it's not the same, we need to add it to an array.
        if (typeof temp[jsonldGraph["@graph"][i]["@id"]][key] !== "undefined" && temp[jsonldGraph["@graph"][i]["@id"]][key] !== jsonldGraph["@graph"][i][key]) {
          if (temp[jsonldGraph["@graph"][i]["@id"]][key] instanceof Array && temp[jsonldGraph["@graph"][i]["@id"]][key].indexOf(jsonldGraph["@graph"][i]["@id"][key]) === -1) {
            temp[jsonldGraph["@graph"][i]["@id"]][key].push(jsonldGraph["@graph"][i]["@id"][key]);
          } else if (typeof temp[jsonldGraph["@graph"][i]["@id"]][key] === "undefined") {
            //if the jey doesn't exist
            var helper = temp[jsonldGraph["@graph"][i]["@id"]][key];
            temp[jsonldGraph["@graph"][i]["@id"]][key] = [helper, jsonldGraph["@graph"][i]["@id"][key]];
          }
        }
        temp[jsonldGraph["@graph"][i]["@id"]][key] = jsonldGraph["@graph"][i][key];
      }
    }
  }
